...
const SYSTEM_MODULES = [
  ...
  { code: "BILLING", name: "Billing and Payments", route: "/billing" },
  { code: "INVENTORY", name: "Inventory", route: "/inventory" },
  { code: "STAFF", name: "Staff", route: "/staff" },
  {
    code: "ATTENDANCE",
    name: "Attendance and Access Control",
    route: "/attendance",
  },
...
async function main() {
  ...
  for (const name of INVENTORY_AREAS) {
    const existing = await prisma.inventoryArea.findFirst({
      where: { hotelId: HOTEL_ID, name },
    });
    if (!existing) {
  ...
  for (const [roleName, modules] of Object.entries(PERMISSION_MATRIX)) {
    ...
    for (const [code, flags] of Object.entries(modules)) {
      ...
      const [canView, canCreate, canUpdate, canDelete, canExport, canManage] =
        flags;
  ...
  if (!process.env.SEED_ADMIN_PASSWORD) {
    console.log(
      "  WARNING: using the default development password. Set SEED_ADMIN_PASSWORD for real environments.",
    );
  }
